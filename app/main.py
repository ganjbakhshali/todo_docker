...
def get_sqlite_connection():
    if not hasattr(threading.current_thread(), "sqlite3_connection"):
        ...
        cursor = threading.current_thread().sqlite3_connection.cursor()
        cursor.execute('CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY, title TEXT, description TEXT, time TEXT, status INTEGER)')
        threading.current_thread().sqlite3_connection.commit()
        cursor.close()
